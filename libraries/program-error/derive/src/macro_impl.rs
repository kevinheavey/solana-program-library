    ...
    pub fn generate_tokens(&mut self) -> proc_macro2::TokenStream {
        match self {
            ...
            Self::PrintProgramError { ident, variants } => {
                print_program_error(ident, variants)
            }
...
pub fn into_program_error(ident: &Ident) -> proc_macro2::TokenStream {
    quote! {
        impl From<#ident> for ::solana_program_error::ProgramError {
...
pub fn decode_error(ident: &Ident) -> proc_macro2::TokenStream {
    quote! {
        impl<T> ::solana_decode_error::DecodeError<T> for #ident {
...
pub fn print_program_error(
    ...
    quote! {
        impl ::solana_program_error::PrintProgramError for #ident {
            fn print<E>(&self)
            where
                E: 'static
                    + std::error::Error
